...
function setupPage() {
    ...
    var leftLabelDiv = document.querySelector('#labels-left');
    var leftLabelDivLoadMargin = getComputedStyle(leftLabelDiv).marginLeft;
    var rightLabelDiv = document.querySelector('#labels-right');
    var rightLabelsMargin = getComputedStyle(rightLabelDiv).marginLeft;
...
function loadImage(image) {
    fetch('assets/img/talkingheads.jpg').then(function (response) {
...
function changeDotMargins(leftLabelDiv, rightLabelDiv) {
    ...
    var dotsContainerDiv = document.getElementById('dots');
    var dotElements = dotsContainerDiv.getElementsByClassName('dot');

    var dotsContainerMargin = getComputedStyle(dotsContainerDiv).marginTop;

    var containerMargin = parseInt(dotsContainerMargin);

    var lengthOfTimeline = timeline.clientHeight - containerMargin;

    var dotCount = dotElements.length;

    var lastDot = dotElements[dotCount - 1];

    var dotHeight = lastDot.clientHeight;
    var adjustedDotHeight = dotHeight * 1.37;

    var start = getDotYear(dotElements[0]);

    var end = getDotYear(lastDot);

    var leftLabels = (0, _from2.default)(leftLabelDiv.querySelectorAll('.label:not(:first-child)'));
    var rightLabels = (0, _from2.default)(rightLabelDiv.querySelectorAll('.label'));

    var yearMargin = getYearMargin(lengthOfTimeline, dotCount, containerMargin, dotHeight, start, end);
    var dotDount = dotElements.length;

    var dotElement = void 0;
    var dotYear = void 0;
    var label = void 0;
    var margin = void 0;
    var lastDotMargin = void 0;
    var labelMargin = void 0;
    var lastMargin = 0;
    var lastYear = start;
    ...
    var lastLabel = dotCount % 2 === 0 ? rightLabels : leftLabels;

    lastLabel.shift().style.marginTop = lastDotMargin + lastMargin + adjustedDotHeight;
}

function getDotYear(dot) {
    var p = dot.getElementsByTagName('p')[0];

    return parseInt(p.textContent);
}

function getYearMargin(lineHeight, dotCount, margin, dotHeight, startDate, endDate) {
    var startOfLastDot = lineHeight - dotHeight - margin;

    var endOfFirstDot = dotHeight + margin;

    var spaceBetween = startOfLastDot - endOfFirstDot - (dotCount - 2) * dotHeight;

    var years = endDate - startDate;

    var marginPerYear = spaceBetween / years;
...
module.exports = !__webpack_require__(68) && !__webpack_require__(140)(function () {
...
// 25.1.2.1.1 %IteratorPrototype%[@@iterator]()
__webpack_require__(65)(IteratorPrototype, __webpack_require__(42)('iterator'), function () { return this; });
